...
export function messageToRole(message: BaseMessage) {
...
export function convertMessagesToParams(messages: BaseMessage[]) {
...
export async function chatCompletions(messages: IMessages[], options: IOptions) {
    const handleResponse = (obj: any) => {
        ...
        if (type == "string") {const message = String(obj).match(/"message":"(.*?)"/gim)
        .toLocaleString().replace('"message":', '').split('[')[0]
            .replace('á', 'a')
            .replace('í', 'i')
            .replace('é', 'e')
            .replace('ó', 'o')
